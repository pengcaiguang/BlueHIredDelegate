...
@property(nonatomic,assign) NSInteger timeSS;
...
-(void)initDrawlView
{
    ...
    if (self.model.data.remark.length>0) {
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:self.remarkLabel.text];
        [string addAttributes:@{NSForegroundColorAttributeName: [UIColor colorWithHexString:@"#666666"]} range:NSMakeRange(0, 5)];
        self.remarkLabel.attributedText = string;
    }else{
...
- (IBAction)touchAllButton:(id)sender {
    ...
    if (self.textField.text.floatValue>self.model.data.chargeMoney.floatValue) {
        self.TitleTF.text = [NSString stringWithFormat:@"手续费%.2f元，实际到账%.2f元",self.model.data.chargeMoney.floatValue,
                             self.textField.text.floatValue-self.model.data.chargeMoney.floatValue];
    }else{
        self.TitleTF.text = [NSString stringWithFormat:@"手续费%.2f元",self.model.data.chargeMoney.floatValue];
    }
        ...
        if (isSuccess) {
            if ([responseObject[@"code"] integerValue] ==0)
            {
                if ([responseObject[@"data"] integerValue] >0) {
                    self.DrawID = responseObject[@"data"];
                    self.SuccessView.hidden = NO;
                    kAppDelegate.userMaterialModel.data.money = [NSString stringWithFormat:@"%.2f",kAppDelegate.userMaterialModel.data.money.floatValue - money];
                    self.timeSS = 3;
...
-(void)timerAction{
    self.timeSS--;
    self.TimerLabel.text = [NSString stringWithFormat:@"%lds后自动跳转至提现详情",(long)self.timeSS];
    if (self.timeSS == 0) {
